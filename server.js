...
app.get('/api/years', (req, res) => {
  ...
  const years = [...new Set(selectedMovies.map(movie => movie.year))].sort((a, b) => a - b);
  
  res.json({ years });
});

app.listen(PORT, () => {
  console.log(`🚀 Damsharas server running on http://localhost:${PORT}`);
  console.log(`📽️  Movie generator with timer, year filter, and hiding functionality`);
}); 